...
const GGML_HEADER: &str = "include/ggml.h";

fn generate_bindings() {
    let ggml_header_path = PathBuf::from(GGML_SOURCE_DIR).join(GGML_HEADER);
    let librs_path = PathBuf::from("src").join("sys.rs");
...
fn build_simple() {
    ...
    let build = builder
        .files([
            PathBuf::from(GGML_SOURCE_DIR).join("src").join("ggml.c"),
            PathBuf::from(GGML_SOURCE_DIR)
                .join("src")
                .join("ggml-alloc.c"),
            // PathBuf::from(GGML_SOURCE_DIR)
            //     .join("src")
        ...
        .include(PathBuf::from(GGML_SOURCE_DIR).join("src"))
        .include(PathBuf::from(GGML_SOURCE_DIR).join("include"));
    #[cfg(not(feature = "no_k_quants"))]
